// subjective-logic-analysis.service.js
(function()
{
  'use strict';

  angular
    .module('app.cctool.services.subjective-logic-analysis')
    .service('subjectiveLogicAnalysisService', subjectiveLogicAnalysisService);

  /* @ngInject */
  function subjectiveLogicAnalysisService($log, $rootScope, $interval, apiResolver)
  {
    /* jshint validthis: true */
    var vm = this;
    
    // Data
    vm.title = 'subjectiveLogicAnalysisService';
                ...
                apiResolver.resolve('cctool.analysis.full@query', {'graphId': graphId, 'analysisType': analysisType}).then(
                  function(data)
                  {
                    ...
                    if (data && data[0])
                    {
                      ...
                      $rootScope.$broadcast('subjectiveLogicAnalysis:hasUpdates',monitoredAnalysis);
