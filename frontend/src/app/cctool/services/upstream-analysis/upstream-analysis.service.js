// upstream-analysis.service.js
(function()
{
  'use strict';

  angular
    .module('app.cctool.services.upstream-analysis')
    .service('upstreamAnalysisService', upstreamAnalysisService);

  /* @ngInject */
  function upstreamAnalysisService($log, $rootScope, $interval, apiResolver)
  {
    /* jshint validthis: true */
    var vm = this;
    
    // Data
    vm.title = 'upstreamAnalysisService';
                ...
                apiResolver.resolve('cctool.analysis.full@query', {'graphId': graphId, 'analysisType': analysisType}).then(
                  function(data)
                  {
                    ...
                    if (data && data[0])
                    {
                      ...
                      $rootScope.$broadcast('upstreamAnalysis:hasUpdates',monitoredAnalysis);
