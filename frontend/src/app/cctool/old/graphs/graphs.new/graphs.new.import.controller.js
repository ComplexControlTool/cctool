...
(function ()
{
    ...
    function ImportGraphController($rootScope, $scope, $q, $timeout, $state, $stateParams, $mdToast, $mdMedia, $mdDialog, $cookies, api, settingsService)
    {
        ...
        function init()
        {
          ...
          vm.gephiToolUrl = 'https://marketplace.gephi.org/plugin/json-exporter/';
          vm.gephiToolInstructionsUrl = 'https://marketplace.gephi.org/how-to-install-a-plugin';
          // Form related
          vm.selectedStep = 0;
          vm.stepProgress = 1;
          vm.processing = false;
        ...
        {
          $scope.$watch('import.stepData[1].data.graphFileName', function(newVal, oldVal)
          {
            if(newVal !== oldVal)
            {
              loadGraphFromJson();
