...
def computeControlConf(graphSet, nodesNo):
	...
	nodesInConf = createInConfSet(graphSet,nodesNo,controlSize,nodesNotInConf)
	...
	for possibleSet in itertools.combinations(freeNodes,freeControl):
			...
			if subMatchingSize == controlSize:
				# If true, nodes with no incoming edges form control configuration.
				output[counter] = set(possibleSet) | nodesInConf
...
def findBestControlConf(controlConfs, controlConfStems, controllability):
	...
	for i in controlConfs:
		...
		rankedControlConfStems.setdefault(tTotal,[]).append(controlConfStems[i])
		i += 1
